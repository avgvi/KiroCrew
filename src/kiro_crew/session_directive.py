...
# What :func:`strip_marker` substitutes for a marker it cannot cut to the end of
# the text without truncating an envelope around it.
_STRIPPED_NOTE = "[directive marker removed]"
...
def strip_marker(text: str) -> str:
    ...
    return out.replace(_SENTINEL, _STRIPPED_NOTE).replace(_REFUSAL_SENTINEL, _STRIPPED_NOTE)
...
def _replace_embedded_payloads(text: str) -> str:
    """Replace every ``SENTINEL{...}`` in *text* with :data:`_STRIPPED_NOTE`."""
    out: list[str] = []
    pos = 0
    while True:
