...
class TestPushRetriesOnRace:
    ...
    def test_a_rewritten_fetch_head_cannot_substitute_the_rebase_base(
        ...
        assert [
            ...
            "--count",
            f"{FETCHED_BASE}..HEAD",
        ] in gits, "the replayed-commit count still reads the mutable ref"
...
class TestOnlyTheCommitThePipelineMadeIsPublished:
    ...
    runs an agent in the SAME clone with ``Bash``/``Edit`` for up to 30 turns, and whose git
    denylist covers only ``push`` and ``remote set-url`` -- so ``git commit --amend`` is
    permitted there. That prompt is now report-only, but a prompt is an instruction rather
    than an enforcement boundary. Nothing compared the published commit against the verified
    one, so "what the pipeline verified" and "what got pushed" could come apart with nothing
    flagging it.
    """
