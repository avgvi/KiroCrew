...
#: Whether this process has already recorded the managed tier as absent. The audit's
#: contract is once per PROCESS, but ``load_security_policy`` is not called once per
#: process: ``mcp_gateway.app_call`` re-runs it on every app callback to compose that
#: call's ceiling, so without this latch a standalone host appended one SEL row per
#: callback for a fact that never changes. Set on the first emit, never cleared.
_ABSENCE_AUDITED = False

#: Whether this process has already warned that an env-tier document composes
#: BENEATH a central authority. Before this change ``KIROCREW_SECURITY_POLICY``
#: outranked the central document and was the documented mid-incident rollback
#: lever; now it only tightens. Nothing else tells an upgraded fleet that its
#: runbook's mental model is wrong until the moment it bites, so the first time the
#: inverted shape actually composes, say so once -- in the log and the SEL.
_ENV_BENEATH_CENTRAL_WARNED = False

#: The ``(authority, lower)`` tier pairs this process has already recorded as
#: intersected. ``compose_tier_ladder`` runs per app callback, and the pairs it folds
#: are fixed for the process lifetime unless a tier appears or disappears, so a row
#: per compose was a row per interaction for a fact that never changed -- burying
#: the one-time absence and env-inversion signals the fleet actually reads. A pair
#: is recorded the first time it composes; an unchanged compose writes nothing, the
#: same rule the refresh path follows for an unchanged poll.
_TIER_INTERSECTS_AUDITED: set = set()
...
#: a different ACL model with no xattr of this name, so the read fails there and the
#: check degrades to the mode bits -- see ``_acl_grants_write_to_non_root``.
...
def _posix_acl_blob_grants_write_to_non_root(blob: bytes) -> bool:
    """Decode an ``acl_ea`` blob; True if a named non-root user/group may write."""
    ...
    for off in range(4, len(blob), 8):
        ...
        if tag not in (_ACL_USER, _ACL_GROUP):
            continue
...
def _audit_managed_absent() -> None:
    ...
    * **Deletion and never-provisioned are the same syscall result.** ``FileNotFoundError``
    ...
    ``_ABSENCE_AUDITED`` latch -- not from :func:`_read_managed_policy`, which also runs
...
def _warn_env_beneath_central_once(authority_tier: str) -> None:
    ...
    """
    global _ENV_BENEATH_CENTRAL_WARNED
    if _ENV_BENEATH_CENTRAL_WARNED:
        return
    _ENV_BENEATH_CENTRAL_WARNED = True
    logger.warning(
...
def _audit_tier_intersect_once(authority_tier: str, lower_tier: str) -> None:
    ...
    one per interaction. See ``_TIER_INTERSECTS_AUDITED``.
    """
    pair = (authority_tier, lower_tier)
    if pair in _TIER_INTERSECTS_AUDITED:
        return
    _TIER_INTERSECTS_AUDITED.add(pair)
...
def load_security_policy(
    ...
    **advisory by default**, so existing installs and policy files keep working
    ...
    global _ABSENCE_AUDITED
    if managed_data is None and _managed_policy_path() is not None and not _ABSENCE_AUDITED:
    ...
    # and ``_declared_distribution`` answers None for it. Falling through the ``or`` chain
    # then let a LOWER tier supply the source -- so a managed document that pinned the
    # channel got a locally chosen endpoint instead, which is the redirection the managed
    # pin exists to refuse. Where the fleet declared the channel and named no address,
    # NOTHING else may supply one: not a lower tier here, and not KIROCREW_POLICY_URL in
    # ``resolve_distribution`` either (the environment is the local account's). The
    # central tier simply stays off until the fleet names a source.
