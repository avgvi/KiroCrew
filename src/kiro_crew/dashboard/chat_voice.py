...
async def api_voice_config(request: web.Request) -> web.Response:
    ...
    # Valid JSON is not necessarily an OBJECT. A list body makes every
    # ``"key" in body`` below a membership test over its ELEMENTS, so
    # ``["system_voice"]`` passes the guard and then raises TypeError on the
    # subscript; a bare string or number fails the same way. Reject the shape
    # once here rather than type-testing at each of the fifteen keys.
...
async def api_voice_synthesize(request: web.Request) -> web.Response:
    ...
    if not isinstance(body, dict):
        # ``body.get`` below is an AttributeError on a list or a string, so the
        # same shape check the config PUT needs applies here too.
        return web.json_response(
            {"error": "body must be a JSON object", "code": "invalid_body"}, status=400
        )
