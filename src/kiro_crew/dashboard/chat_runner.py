...
def refusal_card_text(refusal: "RefusalInfo | None") -> str:
    ...
    if refusal is not None and refusal.explanation:
        lines.append(refusal.explanation)
    if refusal is not None and refusal.recommended_model:
        lines.append(
            f"The provider suggests model '{refusal.recommended_model}' for this request."
        )
    else:
        lines.append(
    ...
    try:
        ...
        if _answer_text:
            ...
            if _stop_reason == STOP_REASON_REFUSAL:
                ...
                slot.append("error", refusal_card_text(_turn_refusal), "msg msg-err")
