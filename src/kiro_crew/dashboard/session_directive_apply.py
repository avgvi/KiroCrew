    ...
    try:
        if kind == "monitor_start":
            result = await _monitor_start(
                state, session_key, args, slot=slot, self_arm_ok=producer_is_user_facing
            )
            ...
            result = await _monitor_update(
                state, session_key, args, self_arm_ok=producer_is_user_facing
            )
...
async def _monitor_start(
    ...
    *,
    ...
    loop, error, status = await authorize_and_add_nudge(
        ...
        # the initiator -- but only for a turn a HUMAN started in that session.
        # A cron injection, a sub-agent sharing the slot, an app-driven or
        # nudge-driven turn runs in the same session without being it, and a
        # loop such a turn armed would be the outsider's loop wearing the
        # member's key. ``producer_is_user_facing`` is the same authenticated-
        # human provenance the set_project / reset_conversation gate uses, so
