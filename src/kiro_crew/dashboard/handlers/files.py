...
async def api_workspaces_create(request: web.Request) -> web.Response:
    ...
    def _mutate_create(doc: dict) -> dict:
        ...
        # replace) is refused, so the atomic rename only ever creates a brand
        # new tree, and a later rollback can only ever remove a tree this
        # request created at a path verified absent inside this critical
        # section.
        if staged_path is not None and install_dst is not None:
            ...
            try:
                os.rename(staged_path, install_dst)
