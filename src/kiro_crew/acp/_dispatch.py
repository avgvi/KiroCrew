...
def _refusal_str(value: object, limit: int) -> str:
    """A provider string bound for the dashboard: str-typed, stripped, capped."""
...
def parse_refusal(params: dict[str, Any]) -> RefusalInfo | None:
    ...
    if not isinstance(refusal, dict):
        refusal = {}
    # Redact BEFORE the cap: a secret straddling the cut would leak its prefix,
    # and a redaction marker can lengthen the text past a pre-redaction cap.
    explanation = _refusal_str(refusal.get("explanation"), 4 * _REFUSAL_EXPLANATION_MAX)
    if explanation:
        explanation, _ = redact_exfiltration_urls(explanation)
        explanation, _ = redact_credentials(explanation)
        explanation = explanation[:_REFUSAL_EXPLANATION_MAX]
    info = RefusalInfo(
        source="metadata",
    ...
    # Category is a closed vocabulary on the service side; a log line naming it
    # is how an operator learns WHICH filter a fleet keeps tripping, without
    # carrying the explanation (prose) or the prompt.
    logger.info(
