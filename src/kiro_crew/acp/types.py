...
class RefusalInfo:
    ...
    value. ``explanation`` is provider text bound for the dashboard: producers
    redact it before it lands here.
    """
    ...
    def to_dict(self) -> dict[str, str]:
        """Wire shape for the dashboard: only the fields that carry a value.

        An absent key means "the provider did not say", which is what the card
        must render as silence -- an empty string would tempt a renderer into
        printing an empty label.
        """
        out: dict[str, str] = {"source": self.source}
        if self.category:
            out["category"] = self.category
        if self.explanation:
            out["explanation"] = self.explanation
        if self.recommended_model:
            out["recommendedModel"] = self.recommended_model
        return out
