...
#: Remedy text for the skip reasons whose fix is a DIFFERENT delivery route.
#: ``no_channel_destination`` and ``channel_upload_unsupported`` both mean this
#: session has no eligible native upload path, and on a messaging channel the
#: renderer's own outbound-image extraction is the route that does work — it
#: uploads a real image referenced inline in the reply, read from the session's
#: working directory. Naming it is what turns a dead end into a next step.
_CHANNEL_SKIP_REMEDY = (
    "for an inline image on a messaging channel, reference it as "
...
def _describe_channel_skip(reason: str) -> str:
    ...
    if reason == "no_channel_destination":
        return f"{head}; {_CHANNEL_SKIP_REMEDY})"
    return f"{head})"
