...
# Set while no deferred fork refresh is pending. The boot-path deferral in
# rebuild_agent_config clears it before scheduling the background pass and the
# pass re-sets it in a finally, so require_fork_governance can hold a
# fork-backed spawn until fork governance has been re-projected.
...
def _refresh_forked_templates(*, gated_off: "frozenset[str] | None" = None) -> None:
    ...
    for fork_name in sorted(forks):
        ...
        if not _binding_corroborates(fork_name):
            ...
            # write to a spec file — governance included.
            continue
