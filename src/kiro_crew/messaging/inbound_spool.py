...
#: gateway calls ``replay_spooled``. Everything else -- the store primitives, the
#: error types, the path -- is module-internal, reached by tests through the
#: module rather than advertised as API.
__all__ = [
    "InboundRoute",
    "ReplayReport",
    "SpooledInbound",
...
def _spool_lock(path: Path) -> Any:
    ...
    ``asyncio.to_thread`` writers. Without this they both read the same snapshot
    and the second atomic replace silently drops the first -- reintroducing the
    loss this module exists to close, one layer up.
...
async def record_refusal(entry: SpooledInbound, *, path: Path | None = None) -> bool:
    """Off-loop :func:`record_refusal_sync`, with an inline fallback.

    The write is disk I/O, so it belongs in a thread. But this is called from a
    shutdown path, where the default executor may already be refusing work
    (``RuntimeError: cannot schedule new futures after shutdown``) — and dropping
    the message because the thread pool closed first would reintroduce exactly
    the loss being fixed. So a failure to hand off falls back to writing inline:
    one small bounded file, on a loop that is being torn down anyway.
    """
    try:
        return await asyncio.to_thread(record_refusal_sync, entry, path=path)
    except RuntimeError:
...
def peek_next(
    *,
    path: Path | None = None,
    now: float | None = None,
    claimable: Callable[[SpooledInbound], bool] | None = None,
) -> SpooledInbound | None:
    """Return the OLDEST actionable entry WITHOUT removing it, or ``None``.

    *claimable* decides, under the lock, whether the caller can act on an entry
    this run; an entry it declines is skipped in place. ``None`` accepts every
    entry. Stale entries are pruned here rather than returned, so the age horizon
    is enforced on the read as well as on the write -- and an all-stale file is
    removed so it is not re-read on every start.
    ...
    try:
        with _spool_lock(target):
            ...
            for entry in remaining:
                if claimable is None or claimable(entry):
                    return entry
            return None
    except SpoolRedirected:
...
def _route_authorized(entry: SpooledInbound, transport: Any) -> bool:
    ...
    if gate is None:
        return False
    try:
        return bool(gate(entry.conversation_id, entry.thread_id or None, principal=entry.user_id))
    except Exception:
        logger.warning(
...
async def replay_spooled(
    ...
    *transports* is the live ``channel_type -> MessagingTransport`` map the host
    ...
    def _actionable(entry: SpooledInbound) -> bool:
        if entry.channel_type not in transports:
            if entry.trace_id not in skipped:
                skipped.add(entry.trace_id)
            ...
            return False
        if entry.trace_id in skipped:
            # Already attempted this pass and left in place (unconfirmed send).
            return False
        return True

    for _ in range(SPOOL_MAX_ENTRIES):
        entry = await asyncio.to_thread(peek_next, path=path, now=now, claimable=_actionable)
        if entry is None:
            break
        ...
        try:
            ...
            continue
        await asyncio.to_thread(remove_entry, entry, path=path)
