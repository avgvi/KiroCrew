...
`initiator_slot_key` only when `producer_is_user_facing` is set — the same
authenticated-human provenance the `set_project` / `reset_conversation` gate
uses. A cron injection, a sub-agent sharing the slot, an app-driven or
nudge-driven turn runs in the member's session without being it, and a loop
such a turn armed would be the outsider's loop wearing the member's key; those
turns pass an empty initiator and the crew/member refusal stands (visibly, via
the notice row). The arm-time admission check for a self-armed loop requires the
...
never the loop's own key echoed back), and only on human-started turns. Incognito and temporary
