...
* **The masks are materialised before launch.** `mount(2)` cannot target an absent path and
  ...
  document (`pat` empty, `vaults.json` `[]`, `settings.json` `{}`) before launch, and refuses
  the spawn when it cannot — a link at any component, a non-regular file at a leaf, or a
  creation failure. The staging directory is a direct child of the data home, so the shared
  `_materialize_maskable_dirs` covers it under its own rule.
...
* **Pre-upgrade orphans are swept.** Older writers staged beside the target, so a crash in
  ...
  `vaults/<id>/` — so the sweep removes them. Because it DELETES, it carries both link
  guards the materialiser does and for a stronger reason: the chain goes through
  `refuse_linked_parent`, and the directory is opened `O_NOFOLLOW | O_DIRECTORY` with every
  `lstat`/`unlink` issued against that pinned descriptor, so neither an agent-planted link
  nor a swap mid-sweep can redirect a delete. A root failing either guard is skipped, not
  escalated to a spawn refusal — skipping removes the hazard, while refusing would break
  every spawn on a host that merely symlinks its legacy home. Regular files only, sparing
