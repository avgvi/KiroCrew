...
**file** (`system.posix_acl_access` is read and its entries walked), never of the
...
judged on its mode bits alone; an unreadable ACL is not evidence of write access. Parsing is **pure-Python and
...
carry a verifying signature or boot aborts; with `require_asymmetric_policy_signature`
also ON, a symmetric HMAC verdict does not count as one.
...
switched a gate off would admit an unsigned plugin. `require_asymmetric_policy_signature`
is the one flag that fails open on a malformed value, because it narrows which proofs
count rather than opening a gate, and reading a typo as ON would reject every
HMAC-signed ceiling a fleet publishes.
