...
| `security` | Denied commands, sensitive paths, approval posture, default approval tier for new chats | `SecurityPanel.tsx`, `PostureDisclosure.tsx` | `handlers/security.py`, `handlers/tailnet.py`, `handlers/file_delivery_consent.py`, `handlers/core.py` (`agent.default_approval_mode`, owner-only write), `chat_handlers.py` (applies the tier at session birth, owner-only) | `GET /api/security/denied-commands`, `PATCH .../builtins/{id}`, `POST .../user`, `GET,POST,DELETE /api/file-delivery/consent`, `GET,PATCH /api/config/kirocrew`, `POST /api/chat/slots` |
