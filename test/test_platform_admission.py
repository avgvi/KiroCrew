...
class TestStrictBooleanFlagCoercion:
    ...
    def test_absent_falls_back_to_the_declared_default(self):
        ...
        assert _coerce_flag(_MISSING, "flag", default=True) is True

    @pytest.mark.parametrize("raw", MALFORMED)
    def test_a_non_boolean_is_never_coerced_and_fails_closed_by_default(self, raw):
        from kiro_crew.platform.admission import _coerce_flag

        # The default ``malformed`` is fail-CLOSED: a present-but-not-boolean value on a
        # security requirement reads as ON, whatever the absent-key default is. Reading
        # "true" (a string) as OFF is how a gate fails open on a typo.
        assert _coerce_flag(raw, "flag") is True
        assert _coerce_flag(raw, "flag", default=False) is True
        # A caller that opts into fail-open gets the permissive answer instead.
        assert _coerce_flag(raw, "flag", malformed=False) is False
        assert _coerce_flag(raw, "flag", default=True, malformed=False) is False
    ...
    def test_an_absent_flag_is_not_reported(self, caplog):
        ...
        with caplog.at_level(_logging.WARNING, logger="kiro_crew.platform.admission"):
            _coerce_flag(_MISSING, "require_asymmetric_policy_signature")
    ...
    def test_an_explicit_null_on_the_asymmetric_flag_stays_permissive(self):
        # The asymmetric flag narrows proofs rather than gating admission, so its
        # malformed direction is OPEN -- ``null`` there leaves HMAC ceilings accepted.
        policy = AdmissionPolicy.from_dict({"require_asymmetric_policy_signature": None})
        assert policy.require_asymmetric_policy_signature is False
