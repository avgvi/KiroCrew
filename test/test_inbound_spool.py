...
def test_the_thread_is_part_of_the_egress_decision(spool_home: Path) -> None:
    """A Discord thread can be revoked while its parent channel is not."""
    _spool(spool_home, channel_type="discord", conversation_id="chan", thread_id="thr", user_id="u")
    transport = _Transport()

    asyncio.run(replay_spooled(transports={"discord": transport}))

    assert transport.send_gate_calls == [("chan", "thr", "u")]
    assert transport.sent[0][2] == "thr"
...
def test_peek_skips_an_entry_the_caller_declines(spool_home: Path) -> None:
    _spool(spool_home, channel_type="discord", message_id="d", text="skip")
    _spool(spool_home, channel_type="telegram", message_id="t", text="take")

    entry = peek_next(path=spool_home, claimable=lambda e: e.channel_type == "telegram")

    assert entry is not None and entry.text == "take"
    assert _texts(spool_home) == ["skip", "take"]
...
def test_whatsapp_declares_a_route_for_dms_only() -> None:
    """A group route would be authorized by a gate that knows nothing of groups.

    WhatsApp's ``may_send_to`` answers from ``dm_policy`` alone; the group roster
    lives in the inbound group gate. Declaring a group route would let a group
    removed or set to ``off`` while the gateway was down receive the notice, so
    groups are not spooled and degrade exactly as before this seam.
    """
    import inspect

    from kiro_crew.whatsapp.transport_dispatch import WhatsAppDispatcher

    source = inspect.getsource(WhatsAppDispatcher._drive)
    assert "inbound_route=(" in source
    assert "None\n                    if group\n                    else InboundRoute(" in source
