...
def _hermetic_governance_globals(monkeypatch, tmp_path):
    ...
    """
    # The absence audit latches once per process; a leaked True would make a test
    # asserting on the record read nothing.
    monkeypatch.setattr(governance, "_ABSENCE_AUDITED", False)
    monkeypatch.setattr(governance, "_ENV_BENEATH_CENTRAL_WARNED", False)
    # ``_LAST_BUNDLED`` is set by any ``load_security_policy(bundled_loader=...)`` on
    # this worker and read by every later ``apply_ceiling``; without the reset a
    # bundled-loader test poisons the ladder of whichever test runs next.
    monkeypatch.setattr(governance, "_LAST_BUNDLED", None)
    # The intersect audit is latched per tier pair; a leaked pair would make a test
    # asserting on that record read nothing.
    monkeypatch.setattr(governance, "_TIER_INTERSECTS_AUDITED", set())
    for var in (
        _POLICY_ENV,
...
class TestAnAbsentManagedTierIsAuditedNotRefused:
    ...
    def test_a_second_load_in_the_same_process_does_not_audit_again(self, monkeypatch, tmp_path):
        ...
        callback on a standalone host. The ``_ABSENCE_AUDITED`` latch is what makes the
        contract true.
        """
        stub = self._recording_sel()
        monkeypatch.setattr(governance, "sel", lambda: stub)
