"""Tests for the opt-in per-session project directory (#8432).
...
class TestSessionProjectDirRefusals:
    ...
    def test_a_key_that_is_not_a_non_empty_string_returns_empty(self, bad_key, tmp_path):
        ...
        A `None` key used to raise inside the broad `except`, which reported the
        caller's mistake as "no per-session directory" -- indistinguishable from
        the feature being off. The guard makes it explicit, and this test fails
...
class TestNamelessCreateThroughTheHandler:
    ...
    @staticmethod
    async def _create(tmp_path, monkeypatch, body, preseed=None, cfg_override=None):
    ...
    @pytest.mark.asyncio
    async def test_the_send_path_deliberately_does_not_derive(self):
