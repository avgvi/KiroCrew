...
class TestMaskableDirsAreMaterializedBeforeTheSpawn:
    ...
    def test_an_existing_directory_is_left_alone_and_not_reported(self, crew_home):
        # Pre-create EVERY precreate leaf, not just one: the invariant under test is
        # "already there -> untouched and unreported", and reporting is per-leaf, so
        # leaving a sibling absent would report that sibling and say nothing about this
        # one. Iterating the tuple keeps it true as leaves are added, the way the two
