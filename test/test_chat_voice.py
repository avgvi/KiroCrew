...
class TestVoiceConfig:
    ...
    async def test_non_object_body_is_rejected_not_a_500(
        ...
        ``None`` reaches the 400 through the older parse-failure branch instead,
        which carries no ``code``; asserted separately rather than folded in, so
        this test cannot be read as claiming a guarantee that branch does not make.
        """
