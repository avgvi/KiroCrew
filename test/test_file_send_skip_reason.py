...
#: both capabilities: Discord honours an inline reference, while WeCom, Weixin,
#: iMessage and Feishu declare ``files_outbound=False`` and leave it literal.
_NO_REMEDY_REASONS = (
    "no_session",
    "restricted_session",
...
class TestDescribeChannelSkip:
    ...
    def test_an_unsupported_channel_is_never_offered_the_inline_route(self, reason: str) -> None:
        ...
        WeCom, Weixin, iMessage and Feishu declare ``files_outbound=False``, so
        their renderers leave an inline reference in the text as literal markup.
        The reason string cannot separate them from Discord, which would honour
        it, so naming the route would be wrong advice half the time.
        """
        out = _describe()(reason)
        assert reason in out
        assert _REMEDY_MARKER not in out
