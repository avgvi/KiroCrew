...
export const loadOlderMessages = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    ...
    try {
      ...
      //
      // If they DO reproduce, the fix is not to reinstate this wait -- it is to
      ...
      // gesture to race. Reinstatement would mean awaiting the quiescence helper
      // in `lib/scrollQuiet` here again, and the `hold=` reading in the debug
      // overlay is what tells the two states apart. Naming the helper's call text
      // in this comment would make the guard test read the prose as the
      // violation, so it is described rather than spelled.
      devOlderSpans(netMs, 0)
