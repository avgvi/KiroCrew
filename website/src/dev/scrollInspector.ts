...
/** Set by the paging thunk, which is the only place that can tell the REQUEST
 *  apart from the deliberate landing hold. Without the split both live inside one
 *  "fetch" number and two very different diagnoses fit it equally well: ~0.1s of
 *  network plus the 2.5s hold cap, or ~2.3s of payload plus a 0.1s hold. */
let olderSpans: { netMs: number; holdMs: number } | null = null

export function devOlderSpans(netMs: number, holdMs: number): void {
  if (!enabled) return
  olderSpans = { netMs, holdMs }
  if (ensureHost()) paint()
}
  ...
  liveEl.textContent =
    ...
    (olderSpans
      ? `\n  net=${(olderSpans.netMs / 1000).toFixed(2)}s` +
        ` hold=${(olderSpans.holdMs / 1000).toFixed(2)}s`
      : '') +
    (moved
