...
export class HeightIndex {
  ...
  setMeasured(index: number, height: number): void {
    ...
    // Every real measurement also teaches the content-aware estimator, so the
    // px-per-character ratio is learned from THIS session's typography rather
    // than a constant baked for one font at one width.
  }
